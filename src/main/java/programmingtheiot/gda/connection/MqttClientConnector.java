...
public class MqttClientConnector implements IPubSubClient, MqttCallbackExtended
{
	...
	private String  host = ConfigConst.DEFAULT_HOST;; //ConfigConst.DEFAULT_HOST;
	private String  protocol = ConfigConst.DEFAULT_MQTT_PROTOCOL;//ConfigConst.DEFAULT_MQTT_PROTOCOL;
	...
	public MqttClientConnector()
	{
		...
		/*
		this.host = "127.0.0.1";//configUtil.getProperty(ConfigConst.MQTT_GATEWAY_SERVICE, ConfigConst.HOST_KEY, ConfigConst.DEFAULT_HOST);
		
		this.port = 8883;//configUtil.getInteger(ConfigConst.MQTT_GATEWAY_SERVICE, ConfigConst.PORT_KEY, ConfigConst.DEFAULT_MQTT_PORT);
		...
		String addr = configUtil.getProperty(ConfigConst.MQTT_GATEWAY_SERVICE, ConfigConst.CERT_FILE_KEY);
		
		_Logger.info("MQTT client connecting to broker: " + this.port + "" + addr);
		
		this.clientID = MqttClient.generateClientId(); //generated client ID;
		Properties properties = new Properties();

		properties.setProperty(SSLSocketFactoryFactory.KEYSTORE, addr);
		// these are specific to the MQTT connection which will be used during connect
		this.persistence = new MemoryPersistence();
		this.connOpts = new MqttConnectOptions();
		this.connOpts.setSSLProperties(properties);
...
//				this.mqttClient = new MqttClient(this.brokerAddr, this.clientID, this.persistence);
	...
	public boolean publishMessage(ResourceNameEnum topicName, String msg, int qos){
		// TODO: determine how verbose my logging should be, especially if this method is called often
		if (topicName == null) {
	...
	{
		_Logger.info("connectionLost method called. Client has successfully disconnected.");
	}
	
	@Override
	public void deliveryComplete(IMqttDeliveryToken token)
	{
		//_Logger.info("deliveryComplete method called. Client has successfully published a message");
	}
	
	@Override
	public void messageArrived(String topic, MqttMessage msg) throws Exception
	{
		_Logger.info("messageArrived method called. a message has arrived");
	}
